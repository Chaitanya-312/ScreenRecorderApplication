...
#heading
lbl=Label(root,text="Screen Recoder",bg="#fff",font="aerial 15 bold")
lbl.pack(pady=20)

image3=PhotoImage(file="rec.png")
Label(root,image=image3,bd=0).pack(pady=30)

#entry
Filename=StringVar()
entry=Entry(root,textvariable=Filename,bg="#90EE90",width=18,font="aerial 15")
entry.place(x=70,y=380)
Filename.set("    Recording Name")


#buttons
start=Button(root,text="START",font="aerial 18",bd=0,command=start_rec)
start.place(x=110,y=230)

image4=PhotoImage(file="pause.png")
pause=Button(root,image=image4,bd=0,bg="#fff",command=pause_rec)
pause.place(x=10,y=450)
